...
public class BlockHandInJar extends Block {
    ...
    public boolean onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult hit) {
        ...
        if (worldIn.getTileEntity(pos) instanceof TileEntityHandInJar) {
            ...
            RegenCap.get(player).ifPresent((data) -> {
                ...
                if (jar.getLindosAmont() >= 100 && data.getState() == PlayerUtil.RegenState.ALIVE && player.isCrouching() && jar.hasHand()) {
                ...
                if (data.getState() != PlayerUtil.RegenState.REGENERATING && !player.isCrouching()) {
    ...
    //MOVED to RenderTypeLookup, fix this!
