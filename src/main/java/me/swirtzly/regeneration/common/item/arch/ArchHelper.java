...
public class ArchHelper {
    ...
    public static void onItemCapabilities(AttachCapabilitiesEvent<ItemStack> event) {
        if (event.getObject().getItem() instanceof BlockItem || event.getObject().getItem() instanceof IDontStore)
            return;
        event.addCapability(CapabilityArch.ARCH_ID, new ArchProvider(new CapabilityArch(event.getObject())));
    }


    public static void onArchUse(PlayerEntity player) {
        if (player.world.isRemote) return;

        ItemStack headSlot = player.getItemStackFromSlot(EquipmentSlotType.HEAD);
        ...
        if (isSafe) {
            ...
            PlayerUtil.sendMessage(player, new TranslationTextComponent("regeneration.messages.arch_fail"), true);
        }
